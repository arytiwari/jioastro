...
-- Reason: Azure OpenAI text-embedding-3-large deployment uses 3072 dimensions by default
...
-- Step 3: Recreate vector similarity index with new dimensions
...
COMMENT ON COLUMN public.knowledge_base.embedding IS 'OpenAI text-embedding-3-large (3072 dimensions) for semantic similarity search';
...
COMMENT ON TABLE public.knowledge_base IS 'Stores astrology rules extracted from knowledge documents with vector embeddings (3072-dim) for semantic search';
